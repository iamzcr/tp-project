define("lib/annotator", [], function(){
    ...
    Annotator.prototype._render = function() {
        ...
        var tpl = [
            ...
            '<textarea></textarea>',
            '<button disabled>OK (Ctrl + Enter)</button>',
            '</div>',
            '</div>'
        ];
    ...
    Annotator.prototype._bindAnnotationEvent = function() {
        ...
        this.annotation.find('.annotation-body textarea').on('keyup', function(evt) {
            var annotation = $(this).closest('.annotation-container');
            if(evt.ctrlKey && evt.keyCode === 13 && this.value.length > 0) {
    ...
    Annotator.prototype._resetAnnotationStatus = function(annotation) {
        ...
        if(completed) {
            ...
            annotation.on("mouseenter", function(){
                annotationBody.stop(true, false).fadeIn();
            });
    ...
    function toPercentString(src, value) {
        if (!value) {
            value = 2;
        }
        return (Math.round(src * 100 * Math.pow(10,value)) / Math.pow(10,value)) + "%";
    }
    ...
    Annotator.prototype._draw = function(annotations) {
        ...
        if(toString.call(annotations) === '[object Array]') {
            for(var i = 0, len = annotations.length; i < len; i++) {
                ...
                this._render();
                this.annotation.css({top: toPercentString(annotation.top), left: toPercentString(annotation.left), width: toPercentString(annotation.width), height: toPercentString(annotation.height), display: 'block'}).addClass('completed');
    ...
    Annotator.prototype.all = function() {
        ...
        for(var i = 0, len = this.annotations.length; i < len; i++) {
            ...
            if(annotation.data('completed')) {
                data.push({top: annotation.position().top / h, left: annotation.position().left / w, width: annotation.width() / w, height: annotation.height() / h, text: annotation.find('.annotation-body textarea')[0].value});
            }
        }
        return data;
    }
    ...
    Annotator.prototype.readonly = function() {
        this.el.off("mousedown");
    }

    Annotator.prototype.editable = function() {
        this._bindEvent();
    }
