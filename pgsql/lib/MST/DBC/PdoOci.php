<?php
/**
 * Class MST_DBC_PdoOci
 * Oracle  PDO 数据库驱动底层封装
 * @package MST_DBC
 */
class MST_DBC_PdoOci implements MST_IDBC {
	...
	public function connect(& $config) {
		...
		try {
			$this->_connector = new PDO(
				"oci:dbname=//{$config['host']}:{$config['port']}/{$config['dbname']};charset={$config['charset']}",
				$config['user'],
				$config['password'],
				self::$_pdoOptions
			);
