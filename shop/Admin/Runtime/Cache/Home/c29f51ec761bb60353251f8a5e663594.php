            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    <form role="form" method="post"  enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label>分类</label>
                            <select class="form-control" name="article[cms_category_id]">
                                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($list["id"]); ?>"><?php echo ($list["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>图片</label>
                        ...
                        <div class="form-group">
                            <label>摘要</label>
                            <textarea class="form-control" rows="5" name="article[summary]"  ></textarea>
                        </div>
                        <div class="form-group">
                            <label>描述</label>
                            <textarea  id = "container"  rows="5" name="article[content]"  ></textarea>
                        </div>
                        <div class="form-group">
                            <label>状态</label>
                            <div class="radio">
                                <label>
                                    <input type="radio" name="article[if_show]" id="optionsRadios1" value="1" checked>显示
                                </label>
                            </div>
                            <div class="radio">
                                <label>
                                    <input type="radio" name="article[if_show]" id="optionsRadios2" value="0">隐藏
                                </label>
                            </div>
                        </div>
                        <input type="submit" class="btn btn-default" value="保存">
                        <a class="btn btn-default" href="<?php echo U('CmsPost/index/');?>">返回</a>
                    </form>
                </div>
            </div>
        </div>
